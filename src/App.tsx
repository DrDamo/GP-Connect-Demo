...
import { MedicationsView } from './components/clinical/MedicationsView'
...
type ActiveTab = 'clinical' | 'raw' | 'validation'
...
export default function App() {
  ...
  const handleLoad = useCallback((text: string, filename: string) => {
    ...
    const record = extractMedications(parsed.data)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-nhs-grey-5">
      {/* NHS-style header */}
      ...
      {!loaded ? (
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-xl space-y-4">
            ...
            >
              Load sample Medications Bundle
            </button>

            <p className="text-xs text-center text-nhs-grey-3">
              Currently supporting: <strong>Medications</strong> domain · more domains coming soon
            </p>
          </div>
        </main>
      ) : (
        /* Main workspace */
        <main className="flex-1 flex flex-col max-w-screen-2xl w-full mx-auto p-4 gap-4 min-h-0">
          {/* Tabs */}
          <div className="flex gap-1 border-b border-nhs-grey-4">
            {(['clinical', 'raw', 'validation'] as ActiveTab[]).map(t => (
              <button
                key={t}
                onClick={() => setTab(t)}
              ...
              >
                {t === 'clinical' ? 'Clinical View' : t === 'raw' ? 'Raw Source' : 'Validation'}
          ...
          <div className="flex-1 min-h-0 overflow-auto">
            {tab === 'clinical' && (
              <div className="bg-white rounded-lg border border-nhs-grey-4 p-4 h-full overflow-auto">
                <MedicationsView record={loaded.record} />
              </div>
            )}
