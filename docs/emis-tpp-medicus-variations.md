...
| `Extension-CareConnect-RelatedProblemHeader-1` (on a consultation topic `List`) | **Gap** | Real, EMIS-only extension found on a consultation *topic* `List` linking that topic to the Problem it's filed under — previously unextracted. `consultations.ts` now reads it and exposes `relatedProblemId`/`relatedProblemDisplay` on the topic; `ConsultationsView` shows "(Problem: …)" next to the topic title, clickable through to the Problems record. Distinct from the same extension's *other*, `Condition`-level use — see the `RelatedProblemHeader` row in §1 (Problems) and its own "Fixes applied" entry below. | Real EMIS bundle: 1 consultation topic resolved to its related problem. |
| `Extension-CareConnect-RelatedProblemHeader-1` (on `Condition` — Group/Combine/Evolve) | **Gap** | A second, unrelated use of the same extension URL: on `Condition`, it records EMIS's Group/Combine/Evolve problem-linking workflows — one parent problem with `type: child` extensions to each child, and every child with a `type: parent` extension back plus `type: sibling` extensions to its co-children, alongside a descriptive `note`. Found in a second real EMIS export (`INT EMIS Prob after combine, evolve and group.json`, Sep 2026) covering all three event types, not in the original audited bundle. `src/fhir/problems.ts` now parses it into `GpConnectProblem.relatedProblems`; `ProblemsView` renders each parent as a single row with children nested underneath (collapsed by default), and lists Parent/Child/Sibling links (with "Go to item →") in the problem detail panel. The Builder's Problems section can now generate it too — a **Combine / Group / Evolve…** multi-select action (`ProblemForm.tsx`) drives `generateProblems` (`builder/generate/problems.ts`) to emit the matching extensions and EMIS-style notes. See `src/training-content/11_problems.md` ("EMIS/TPP Group, Combine & Evolve"). | Real EMIS bundle: 9 Conditions across 3 groups (1 group of 3, 1 combine of 2, 1 evolve of 2) — all links and notes round-tripped correctly. |
...
| `Extension-CareConnect-RelatedProblemHeader-1` (parent/child problem hierarchy, on `Condition`) | Real — records Group/Combine/Evolve problem-linking events; extracted, rendered as a collapsed parent+children tree, and generatable from the Builder | Not present in the real TPP bundle (only 2 Conditions, too small to rule out) | not observed | Prior audit note claimed this was a "TPP" extension not yet extracted; a first-pass grep of the original audited bundle found only a *different* use of the same extension URL, on a consultation topic `List` (see the "Fixes applied" table above) — leading an earlier version of this doc to conclude the `Condition`-level parent/child hierarchy was unconfirmed/not present. A second real EMIS export (`INT EMIS Prob after combine, evolve and group.json`, Sep 2026) settled it: the extension **does** appear directly on `Condition`, exactly as originally described — one parent with `type: child` links to each child, each child with a `type: parent` link back plus `type: sibling` links to its co-children, covering all three of EMIS's Group/Combine/Evolve workflows. Now implemented end-to-end: `src/fhir/problems.ts` (extraction) → `ProblemsView.tsx` (collapsed tree UI + ancestry navigation in the detail panel) → `builder/forms/ProblemForm.tsx` + `builder/generate/problems.ts` (Combine/Group/Evolve builder action). See the "Fixes applied" table above and `src/training-content/11_problems.md`. |
...
- **`Extension-CareConnect-RelatedProblemHeader-1`** (parent/child problem hierarchy) — claimed
  ...
  **RESOLVED against a real bundle (Sep 2026):** it's real — first found once in the original
  audited EMIS bundle as a top-level extension on a consultation topic `List` (not `Condition`),
  at `https://fhir.hl7.org.uk/STU3/StructureDefinition/Extension-CareConnect-RelatedProblemHeader-1`
  — an EMIS-observed extension, not TPP (so the prior audit's TPP attribution was backwards) and
  not present in the real TPP bundle. **FULLY RESOLVED AND IMPLEMENTED (Sep 2026, later same
  month):** a second real EMIS export confirmed the extension also has a genuinely distinct
  `Condition`-level use for EMIS's Group/Combine/Evolve problem-linking workflows (parent/child/
  sibling `Condition`-to-`Condition` links), which is now extracted, rendered as a collapsed
  problem tree, and buildable from the Builder — see the `Condition` row for this extension in §1
  and its "Fixes applied" table entry above. No longer an open question.
